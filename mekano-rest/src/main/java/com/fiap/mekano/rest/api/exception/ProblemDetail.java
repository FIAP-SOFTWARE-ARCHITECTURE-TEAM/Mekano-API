...
public record ProblemDetail(
        @Schema(description = "URI que identifica o tipo do problema (RFC 7807)", example = "about:blank")
        String type,

        @Schema(description = "Resumo legível do problema", example = "Not Found")
        String title,

        @Schema(description = "Código de status HTTP", example = "404")
        int status,

        @Schema(description = "Explicação específica desta ocorrência", example = "Usuário não encontrado")
        String detail,

        @Schema(description = "URI que identifica a ocorrência específica (opcional)", example = "/api/v1/users/123")
        String instance
) {
