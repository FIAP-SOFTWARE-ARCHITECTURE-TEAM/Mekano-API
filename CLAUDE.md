...
- PostgreSQL 16-alpine (prod/dev), docker-compose na raiz
- **H2 in-memory** (testes) — modo `MODE=PostgreSQL` no JDC URL, sem Docker
- Flyway: `V1__` a `V5__` em `mekano-infrastructure/src/main/resources/db/migration/`
...
### H2 Compatibility — Regras para Migrations

Testes rodam em H2 (não PostgreSQL) com Hibernate `drop-and-create` — **Flyway não executa em testes**. Migrations rodam apenas em dev/prod (PostgreSQL). Ainda assim, evite os padrões abaixo para manter compatibilidade caso o contexto mude:

1. **`BIGSERIAL`** — use `BIGINT GENERATED BY DEFAULT AS IDENTITY` (SQL standard, compatível com ambos)
2. **Múltiplos `ADD COLUMN` com vírgula** — H2 não aceita `ALTER TABLE t ADD COLUMN a INT, ADD COLUMN b INT`. Use statements separados: `ALTER TABLE t ADD COLUMN a INT; ALTER TABLE t ADD COLUMN b INT;`

Tipos OK: `UUID`, `TIMESTAMP`, `BOOLEAN`, `VARCHAR`, `INT`, `BIGINT`, `NOW()`, `DEFAULT`, `CREATE INDEX`, `ALTER TABLE RENAME COLUMN`, `DROP/ADD CONSTRAINT`.
