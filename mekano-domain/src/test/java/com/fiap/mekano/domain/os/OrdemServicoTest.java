...
class OrdemServicoTest {
    ...
    void deveCriarNovaOsAberta() {
        ...
        assertEquals(OsStatus.AGUARDANDO_APROVACAO, os.getStatus());
    }
    ...
    void deveRestaurarOs() {
        ...
        OrdemServico os = OrdemServico.restaurar(uuid, OsStatus.RECEBIDA);
    ...
    void naoDeveAprovarAntesDeOrcar() {
        OrdemServico os = OrdemServico.restaurar(UUID.randomUUID(), OsStatus.EM_DIAGNOSTICO);

        IllegalStateException exception = assertThrows(IllegalStateException.class, os::aprovar);

        assertTrue(exception.getMessage().contains("Transição inválida"));
        assertEquals(OsStatus.EM_DIAGNOSTICO, os.getStatus());
    }
